...
class RecommendationService {
    ...
    async getRecommendations(preferences) {
        ...
        if (shirts.length === 0)
            console.warn("⚠️ No shirts found in inventory matching keywords: tshirt, shirt");
        if (jackets.length === 0)
            console.warn("⚠️ No jackets found in inventory matching keywords: jacket, hoodie, sweater, puffer");
        if (jeans.length === 0)
            console.warn("⚠️ No jeans found in inventory matching keywords: jean, pant, cargo, trouser");
        if (shoes.length === 0)
            console.warn("⚠️ No shoes found in inventory matching keywords: shoe, sneaker, oxford");
        return {
            shirts,
            jackets,
            jeans,
